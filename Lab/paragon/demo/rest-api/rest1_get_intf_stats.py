...
if r.status_code==200 and r_json['status'] == 'AUTHENTICATED':
    ...
    data1= {
        ...
        'interface': [
            {'name': 'ge-0/0/3.0', 'node': {'topoObjectType':'node', 'hostName': 'p3'} },
            {'name': 'ge-0/0/1.0', 'node': {'topoObjectType':'node', 'hostName': 'p3'} },
            {'name': 'ge-0/0/1.0', 'node': {'topoObjectType':'node', 'hostName': 'pe1'} }
        ]   
    }
    d1 = json.dumps(data1).encode('utf-8')
